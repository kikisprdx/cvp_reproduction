...
class SVPHead(nn.Module):
    ...
    def loss(self,x,  y: torch.Tensor, y_hat: torch.Tensor, tau):
        ...
        num = [[F.cosine_similarity(x_i.unsqueeze(0), x_j.unsqueeze(0)) for x_j in x] for x_i in x]  
        num = num / tau

        den = [[F.cosine_similarity(x_i.unsqueeze(0), x_j.unsqueeze(0)) for x_j in x] for x_i in x]
        den = den / tau
        den = torch.sum(den)
